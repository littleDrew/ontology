...
## 13. 功能视角深度 Review（聚焦思路、深度与细腻度）

> 本章仅从功能视角审视当前方案（DFX 暂不展开），回答三个问题：
> 1) 文档思路是否完整；2) 分析深度是否足够；3) 细腻度是否可直接指导实施。

### 13.1 总体结论

当前文档已具备“可研级 + 架构级”基础，但距离“可直接交付研发实现”的功能规格仍有缺口：
- **思路完整性**：较好（对象监控的核心链路已覆盖）。
- **分析深度**：中上（已有竞品、模式、架构，但功能契约不够可执行）。
- **细腻度**：中（部分章节仍是原则级，缺少功能边界、优先级与验收语义）。

### 13.2 功能架构成熟度评估（按域）

| 功能域 | 当前成熟度 | 主要问题 | 优化方向 |
|---|---|---|---|
| Ontology/对象模型 | 中 | ObjectType/LinkType 与规则作用域映射不够细 | 增加“对象作用域解析规则”与冲突优先级 |
| Monitor DSL | 中 | 语法有了，语义约束与错误模型不足 | 补齐 DSL 语义规范、静态检查与错误码 |
| Rule Evaluation | 中上 | 阈值/时长具备，但多条件组合与冲突策略不清晰 | 定义条件计算顺序、短路逻辑、冲突仲裁 |
| Notification | 中 | 通道策略有描述，但告警治理能力不足 | 增加抑制、合并、升级、静默窗口 |
| Activity/Audit | 中上 | 审计链路有设计，但功能查询维度不足 | 增加调查视图（按对象/规则/租户/时间） |
| Rule Lifecycle | 中 | 创建-发布-回滚有，但缺灰度与模拟评估闭环 | 增加 dry-run、canary、自动回滚流程 |

### 13.3 当前文档在“功能思路”上的优点

1. **监控核心抽象正确**：对象、输入、条件、评估、通知、动作、活动日志链路完整。
2. **模式拆分正确**：复制/非复制/混合三种数据模式切分清晰。
3. **行业问题意识较强**：持续时长规则、审计回放、多租户配额均有覆盖。

### 13.4 功能深度不足点（需要修改）

#### F1. 规则功能分类不足（影响可实施性）
当前规则仅展示阈值与持续时长，缺少完整规则族定义。建议新增四类：
- `Threshold Rule`（阈值）
- `Duration Rule`（持续时长）
- `Composite Rule`（多条件组合）
- `Sequence Rule`（事件序列）

并明确每类规则的：输入要求、状态要求、计算复杂度、预期延迟。

#### F2. 规则冲突与优先级缺失
同一对象可能命中多规则，当前未定义冲突裁决。建议新增：
- 优先级模型：`P1 > P2 > P3 > P4` + 规则权重
- 冲突策略：`highest-severity-wins` / `multi-fire` / `first-match`
- 去重域：对象级、规则级、租户级三层 dedup scope

#### F3. 通知功能“业务可用性”描述不足
目前更偏“技术投递”，需补齐“业务告警治理”：
- 告警抑制（Suppression）
- 告警聚合（Aggregation）
- 告警升级（Escalation）
- 维护静默（Maintenance Window）
- 值班路由（On-call Rotation）

#### F4. 活动日志缺少“调查闭环”功能
Activity 目前偏记录，建议补齐调查功能：
- 关联查询：从一个告警追溯到输入快照、规则版本、通知动作
- 对比查询：同规则在不同租户/时段命中差异
- 可解释性：展示 condition 计算路径（哪一条子条件为 true/false）

#### F5. 规则生命周期缺“预演机制”
缺少上线前验证闭环。建议新增三步：
1. `dry-run`（仅评估不触发动作）
2. `canary`（灰度对象集 5%）
3. `full rollout`（监控指标达标后全量）

### 13.5 结构化修订建议（直接修改到现有章节）

#### 对第10章（DSL/API）补充内容
- 新增 `10.7 DSL 语义约束`：类型系统、函数白名单、复杂度阈值。
- 新增 `10.8 规则冲突裁决`：优先级与去重范围。
- 新增 `10.9 API 错误模型`：校验失败、版本冲突、权限不足、幂等冲突。

#### 对第11章（Phase 1）补充内容
- 新增“功能验收清单（按功能域）”，而非仅性能指标：
  - 规则创建/发布/回滚
  - 告警抑制/聚合/升级
  - Activity 可追溯链路完整性
  - dry-run/canary/full 三阶段发布流程

#### 对第12章（架构评估）补充内容
- 从“问题列表”升级为“问题 -> 功能改造项 -> 验收标准 -> owner”。
- 每个功能改造项定义完成条件与回归测试用例。

### 13.6 优化后的功能蓝图（V2）

```mermaid
flowchart TB
    subgraph Design[规则设计与治理]
      D1[DSL Editor]
      D2[Static Validator]
      D3[Dry-run Simulator]
      D4[Canary Publisher]
    end
    ...
    subgraph Runtime[运行时]
      ...
      R5[Notification Policy Engine]
    end

    subgraph Ops[运维与调查]
      O1[Activity Timeline]
      O2[Root-cause Query]
      O3[Audit Export]
    end
...
1. 规则功能
- 支持 4 类规则：阈值/持续时长/组合/序列。
- 规则冲突有确定性结果（同输入多次评估结果一致）。

2. 通知功能
- 支持抑制、聚合、升级、静默窗口。
- 通知链路可追踪到 dedupKey 与策略命中原因。

3. 调查功能
- 任意告警可在 3 跳内追溯：对象快照 -> 规则版本 -> 通知动作。
- 支持按租户/规则/对象维度回放与对比。

4. 生命周期功能
- 每条新规则发布必须经过 dry-run 与 canary。
- canary 失败自动回滚并留下审计记录。

### 13.8 下一轮修订落地清单（建议 2 周）

- 第 1 周：补全 DSL 语义、冲突裁决、通知治理策略。
- 第 2 周：补全 Activity 调查视图、发布预演机制与功能验收用例。

> 以上修订完成后，文档可从“架构方案”提升为“功能规格 + 实施指引”级别，可直接指导 Phase 1 研发。
